 ...
 *
 */
public class ClientFxGUINew extends Application implements ClientUserInterface {

	private static Log log = LogFactory.getLog(ClientFxGUINew.class);
	
	//test

	private Stage stage;
	private static LogInGuiControllerNew lc;
	private static LoggedInGuiControllerNew lc2;
	...
	public void start(Stage primaryStage) throws Exception {
		FXMLLoader loader = new FXMLLoader(getClass().getResource("LogInGuiNew.fxml"));
		Parent root = loader.load();
		LogInGuiControllerNew lc = (LogInGuiControllerNew) loader.getController();
		lc.setAppController(this);
		primaryStage.setTitle("Anmelden");
		primaryStage.setScene(new Scene(root, 300, 420));
		root.setStyle("-fx-background-color: #99CBBC");
		stage = primaryStage;
		primaryStage.show();
	}
	...
	public void switchToLogInGui(){
		try {
			FXMLLoader loader = new FXMLLoader(getClass().getResource("LogInGuiNew.fxml"));
			Parent root = loader.load();
			LogInGuiControllerNew lc = loader.getController();
			lc.setAppController(this);
			Platform.runLater(new Runnable() {
				@Override
				public void run() {
